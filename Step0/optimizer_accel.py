...
if HAS_NUMBA:
    ...
    @njit(cache=True)
    def _eval_kernel_sink(ind, sts, nps, lens, ac, ad, bxy, pop,
                          r0,r1,r2,r3, xm0,xm1,xm2,xm3, ym0,ym1,ym2,ym3,
                          gres, spd, maxt, tlim):
        """
        v4 扩展内核: 与 _eval_kernel 相同的 Phase 1+2 计算, 但额外返回
        per-resident 到达时刻数组, 供后续的 sink Phase 3 使用。

        返回:
            tt_weighted – 人口加权总步行时间
            tr_walk     – 步行阶段累积风险
            arrivals    – (n,) 每位居民到达上车点的秒数
            infeasible  – 0=可行, 1=超时/不可行 (替代原 1e18 哨兵)
        """
        n = len(ind)
        ...
        for m in range(tlim):
            ...
            for i in range(n):
                ...
                rt += rv * pop[i]
            tr += rt * 60.
        return tt, tr, arrivals, 0

else:
    _eval_kernel = None
    _eval_kernel_sink = None
...
# ============================================================
#  v4 加速评估 (支持 sink + shelter_mapping)
# ============================================================
def make_evaluate_accel_v4(res_x, res_y, pop_arr, bus_xy, road_paths,
                           risk_arrays, x_mins, y_maxs,
                           speed=WALK_SPEED, max_time=DEFAULT_MAX_TIME,
    ...
    """
    v4 加速评估工厂 — 同时支持 Numba 向量化 Phase 2 和 sink Phase 3。

    与 make_evaluate_accel (v3) 的差异:
      v3: Phase 1+2 Numba → 返回 (walk_time, walk_risk), 不支持 sink
      v4: Phase 1+2 Numba (返回 arrival_times) → Phase 3 纯 Python sink
          → 返回 (walk_time + sink_extra, walk_risk + queue_risk)

    性能分析 (N=200 居民):
      v3 Phase 2 (Numba 向量化): ~5 ms  (vs 原 Python shapely: ~75 ms)
      v4 Phase 3 (纯 Python sink): ~3 ms
      v4 总计: ~8 ms  vs  标准 optimizer.make_evaluate: ~80 ms
      加速比: 约 10×

    Pareto 前沿与标准 make_evaluate 数学等价 (float64 精度)。

    参数同 optimizer.make_evaluate, 包括 v5.3 多候选参数:
        shelter_mapping       – (legacy v5.1) 1-to-1 静态映射
        shelter_mapping_multi – (v5.3) Top K 候选级联
        shelter_capacities    – (v5.3) 全局共享容量数组
    """
    n = len(res_x); tlim = int(max_time / 60)
    pop_np = np.asarray(pop_arr, np.float64)
    bus_np = np.asarray(bus_xy, np.float64)
    ...
    if use_sink:
        ...
        sink_model = PickupSinkModel(
            bus_np, risk_arrays, x_mins, y_maxs, sink_config,
            shelter_mapping=shelter_mapping,
            shelter_mapping_multi=shelter_mapping_multi,
            shelter_capacities=shelter_capacities,
        )
        if shelter_mapping_multi is not None:
            mode_str = f"multi-shelter cascade (Top K)"
        elif shelter_mapping is not None:
            mode_str = "1-to-1 mapping"
        else:
            mode_str = "fixed 30km"
        print(f"   🚏 v4: Numba Phase 2 + Python Phase 3 sink ({mode_str})")
    else:
        print(f"   🚏 v4: Numba Phase 2 only (no sink)")

    # ── Numba 预热 ──
    if HAS_NUMBA and _eval_kernel_sink is not None:
        print("   🔥 Numba: compiling v4 kernel ...")
        t0 = time.time()
        _d1 = np.zeros(1, np.int64); _d2 = np.array([2], np.int64)
        _dl = np.array([1.]); _dc = np.array([[0.,0.],[1.,1.]])
        _dd = np.array([0.,1.414]); _db = np.array([[0.,0.]]); _dp = np.array([1.])
        _dr = np.zeros((10,10))
        _eval_kernel_sink(_d1,_d1,_d2,_dl,_dc,_dd,_db,_dp,
                          _dr,_dr,_dr,_dr, 0.,0.,0.,0., 1.,1.,1.,1.,
                          400.,2.,2700.,1)
        print(f"   ✅ Numba v4: compiled ({time.time()-t0:.1f}s, cached)")
    ...
    def evaluate(ind):
        ...
        # ── Phase 1+2: Numba JIT (walk_time_w, walk_risk, arrivals) ──
        if HAS_NUMBA and _eval_kernel_sink is not None:
            tt, tr, arrivals, infeas = _eval_kernel_sink(
                ia, sts, nps, lens, ac, ad, bus_np, pop_np,
                ra[0],ra[1],ra[2],ra[3],
                x_mins[0],x_mins[1],x_mins[2],x_mins[3],
                y_maxs[0],y_maxs[1],y_maxs[2],y_maxs[3],
                float(GRID_RES), float(speed), float(max_time), tlim)
            ...
            # fallback: 复用 _eval_fallback, 再额外计算 arrivals
            tt, tr = _eval_fallback(ia, sts, nps, lens, ac, ad, bus_np, pop_np,
                                    ra, x_mins, y_maxs, GRID_RES, speed,
                                    max_time, tlim, n)
            if not np.isfinite(tt):
                return (np.inf, np.inf)
            arrivals = lens / speed

        # ── Phase 2 only 模式: 直接返回 ──
        if not use_sink:
            return float(tt), float(tr)

        # ── Phase 3: 纯 Python sink (O(events), ~3 ms) ──
        try:
        ...
        # ── 合成总目标 (与 optimizer.make_evaluate 相同公式) ──
        max_arrival = float(arrivals.max()) if len(arrivals) else 0.0
        sink_extra_time = max(0.0, T_total - max_arrival) * float(pop_np.sum())
        total_time_obj = float(tt) + sink_extra_time
        return total_time_obj, float(R_total)
